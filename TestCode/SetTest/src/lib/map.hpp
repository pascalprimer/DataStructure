...
public:
	...
	public:
		...
		iterator(Node *_p, const map *_which): p(_p), which(_which) {
		}
	...
	public:
		...
		const_iterator(Node *_p, const map *_which): p(_p), which(_which) {
		}
	...
	friend BinaryOutput &print_tree(BinaryOutput &out, const map &obj, Node *now) {
		out << now -> value -> first;
...
//cout << "up" << endl;
	...
	friend BinaryOutput &operator << (BinaryOutput &out, const map &obj) {
		out << (int)obj.cnt;
		if (obj.cnt) {
	...
	friend BinaryInput &operator >> (BinaryInput &in, map &obj) {
	 ...
	 */
	map(): cnt(0), root(nullptr), max_p(nullptr), min_p(nullptr), nil(new Node()) {
	}
	map(const map &other): cnt(other.cnt), nil(new Node()) {
	...
	inline pair<iterator, bool> insert(const value_type &value) {
		Node *nownode = find_node(root, value.first);
		if (nownode) {
...
/*if (!pos.p) {
std::cerr << "erase nullptr" << std::endl;
} else {
	std::cerr << "erase not here" << std::endl;
}*/
			throw InvalidIterator();
		}
		delete_node(pos.p);
...
private:
	...
	struct Node {
		...
		Node(const value_type &val, const Color& _col):
			value(new value_type(val)), father(nullptr), child{nullptr, nullptr}, prev(nullptr), next(nullptr), col(_col) {
		}
		Node(value_type *val, const Color& _col):
			value(new value_type(*val)), father(nullptr), child{nullptr, nullptr}, prev(nullptr), next(nullptr), col(_col) {
		}
	...
	inline Node *find_node(Node *u, const Key &_key) const {
		...
		if (Compare()(u -> value -> first, _key)) {
			return find_node(u -> child[1], _key);
		}
		if (Compare()(_key, u -> value -> first)) {
	...
	inline Node *insert_node(Node *u, const value_type &value) {
		...
		while (u != nil) {
			pre = u;
			if (Compare()(u -> value -> first, newnode -> value -> first)) {
				u = u -> child[1];
			} else {
				u = u -> child[0];
			}
		}
		int k = Compare()(pre -> value -> first, newnode -> value -> first);
		pre -> child[k] = newnode;
		newnode -> father = pre;
		newnode -> child[0] = newnode -> child[1] = nil;
		newnode -> prev = find_smaller_node(root, newnode -> value -> first);
		newnode -> next = find_bigger_node(root, newnode -> value -> first);
		if (newnode -> prev) {
